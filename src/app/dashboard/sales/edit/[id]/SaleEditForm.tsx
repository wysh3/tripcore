...
export default function SaleEditForm({ sale }: SaleEditFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success("Sale campaign updated!");
    ...
    <div className="space-y-6 max-w-3xl mx-auto font-sans pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Edit Sale Campaign</h1>
            <p className="text-gray-500 text-sm mt-1">Update your promotional offer or seasonal sale.</p>
          </div>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-2">
            ...
            <input 
              ...
              readOnly
            />
            <p className="text-[11px] text-gray-400">The URL slug is automatically updated from the sale name.</p>
          </div>
